...
<div class="container">
    <div class="filtro-mobile d-md-none">
        ...
        <div class="titulo-filtro__wrapper">
            <p class="titulo-mobile">Todos os produtos</p>
            <p class="limpar-filtro">
                Limpar filtro
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 11.83 12.12" xml:space="preserve"><path d="m7.17 6.05 4.38-4.49c.18-.19.29-.44.29-.69a.8.8 0 0 0-.26-.61c-.35-.36-.92-.35-1.28.02L5.91 4.77 1.53.28A.894.894 0 0 0 .25.26c-.34.35-.33.94.02 1.31l4.38 4.49-4.38 4.49a.94.94 0 0 0-.02 1.31c.35.35.92.34 1.28-.02l4.38-4.49 4.38 4.49c.18.19.43.29.67.29.23 0 .45-.09.6-.27a.94.94 0 0 0-.02-1.31l-4.37-4.5z"/></svg>
            </p>
        </div>
    </div>

    <div class="tab-produtos">
        <a href="javascript: void(0);" class="link voltar">Filtros</a>
        <a href="javascript: void(0);">
            <label class="tab todos ativo">
                Todos os produtos
                <input type="radio" name="produto" value="todos" checked="checked" />
                <span></span>
            </label>
        </a>
        <a href="javascript: void(0);">
    ...
    <div class="lista-produtos">
        <div class="row">
            ...
            ?>
                <div class="<?=$produto['destaque'] ? "col-12" : "col-6"?> col-md-4 col-lg-3 produto <?=$produto['categoria'];?>">
                    <div class="fotos">
                        <?php
                            foreach($produto['fotos'] as $key_foto => $foto){
                                echo '<img class="img-fluid" loading="lazy" src="' . do_shortcode("[img-url]").$foto . '" alt="' . $produto['nome'] . '" />';
                            }
                        ?>
                    </div>
                    ...
                    <div class="locais">
                        <?php
                            foreach($produto['locais'] as $key_local => $local){
                                if($local == 'panela'){
                                    echo '<div>
                    ...
                    <p class="paragrafo"><?=$produto['descricao_curta'];?></p>
                    <a href="javascript: void(0);" data-options='{"touch" : false, "arrows" : false, "infobar" : false}' data-fancybox="modal-produto" data-src="#modal-produto-<?=$key;?>" class="link link-arrow" title="Mais informacões">Mais informacões</a>
                </div>
            <?php
            }
            ?>
    ...
    foreach($produtos as $key => $produto){
    ?>
        <div style="display: none;" id="formulario-<?=$key;?>"><h3><?=$key;?></h3></div>
        <div id="modal-produto-<?=$key;?>" class="modal-produto" style="display:none;">
            <div class="produto">
                <div class="fotos">
                    <?php
                        echo '<img class="img-fluid" loading="lazy" src="' . do_shortcode("[img-url]").$produto['fotos'][0]. '" alt="' . $produto['nome'] . '" />';
                    ?>
                </div>
                <h2 class="nome"><?=$produto['nome'];?></h2>
                <div class="locais">
                ...
                <?php
                    if($produto['diferenciais']){
                ?>
                    <h3 class="sub-titulo mt-5">Diferenciais</h3>
                    <p class="paragrafo">
                ...
                <?php
                    if($produto['receitas']){
                ?>
                    <h3 class="sub-titulo mt-5">Sugestões de receitas</h3>
                    <p class="paragrafo">
                        <?=$produto['receitas'];?>
                    </p>
                <?php
                ...
                ?>
                <a href="javascript: void(0);" class="btn btn-outline mt-5" title="Quero conhecer">Quero conhecer</a>
            </div>
        </div>
    <?php
    }
